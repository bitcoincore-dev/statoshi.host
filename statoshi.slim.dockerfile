#BEGIN statoshi.slim.dockerfile
RUN df -H
###########################################
...
WORKDIR /
# Change to your fork
RUN git clone -b master https://github.com/bitcoincore-dev/stats.bitcoincore.dev --depth 1 stats.bitcoincore.dev && mkdir -p /stats.bitcoincore.dev/depends/SDKs
