...
RUN cd /statoshi && make download -C /statoshi/depends

RUN cd /statoshi && ./autogen.sh

RUN cd /statoshi && ./configure --disable-wallet --disable-tests --disable-hardening --disable-man --enable-util-cli --enable-util-tx --with-gui=no --without-miniupnpc --disable-bench # --prefix=/statoshi/depends/x86_64-pc-linux-gnu
...
RUN cp /statoshi/statoshi.daemon.py /
